...
| 删除消息 | 悬停自己发送的消息，点气泡右上角 × 即可删除（文字/图片/文件均可）；删除图片、文件消息会同步清理服务端对应文件，全员实时移除 |
...
- 只能删除**自己发送**的消息（服务端按消息记录的 `userId` 校验，他人消息直接拒绝）。
- `messages.jsonl` 保持 append-only：删除时追加一条**墓碑记录**（`{ type: 'msg:deleted', delId }`，自身占用 id 序列防止重启后新消息复用已删除的 id），启动加载时按墓碑过滤，历史中不再出现。
- 删除**图片消息**会删除 `uploads/` 下的原图文件；删除**文件消息**会删除 `files/` 下的文件并从 `files.json` 注册表移除条目（下载立即 404）。
- 删除成功后广播 `msg:deleted`，所有在线成员的界面实时移除该消息。
...
- 客户端 → 服务端：`msg:send`（文字 / 图片 / 文件）、`msg:delete`（删除自己的消息）、`history:load`
- 服务端 → 客户端：`init`、`msg:new`、`msg:deleted`、`sys`、`presence`
...
会拉起临时实例（另起一个短 TTL 实例验证文件过期清理），用 `X-Forwarded-For` 模拟不同内网 IP，覆盖：按 IP 自动命名、空名字登录、同主机号冲突加序号、手动名字不被覆盖、Cookie 自动登录、文字与图片收发、类型白名单、文件上传/发送/下载、大小上限、文件 24 小时过期与服务端自动清理、删除自己的消息（权限校验、图片/文件服务端清理、墓碑持久化）、历史分页、落盘与重连恢复，共 53 项检查。
